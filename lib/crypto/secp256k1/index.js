...
export function publicKeyTweakAdd(publicKey, tweak) {
    const point = ec.keyFromPublic(publicKey)
    if (point === null) throw new Error(messages.EC_PUBLIC_KEY_PARSE_FAIL)

    tweak = bufferToBigNumber(tweak)
    if (tweak.isZero()) throw new Error(messages.EC_PUBLIC_KEY_TWEAK_ADD_FAIL)

    const newPub = g.mul(tweak).add(point)
    if (newPub.inf) throw new Error(messages.EC_PUBLIC_KEY_TWEAK_ADD_FAIL)
