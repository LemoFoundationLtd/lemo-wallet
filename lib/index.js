...
class LemoWallet {
     ...
     * Sign a transaction
     * @param {string} address Creator address
     * @param {object|LemoTx} txConfig transaction information
     * @param {string} password
     * @return {LemoTx} Signed tx
     */
    sign(address, txConfig, password) {
        this.pwd.verifyPassword(password)
        const info = this.accounts.decodeAccount(address, password)
        const newTx = new LemoTx(txConfig)
        if (!txConfig.chainID) {
        ...
        if (!newTx.from) {
            ...
            console.warn(`Signed a tx which "from" is "${newTx.from}" by account "${address}"`)
        }
        newTx.signWith(info.privKey)
        return newTx
    }
    ...
    modifyPassword(oldPassword, newPassword) {
        ...
        this.accounts.reEncrypt(oldPassword, newPassword)
    }
