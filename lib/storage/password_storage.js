...
export default class {
    ...
    modifyPassword(oldPassword, newPassword) {
        ...
        let list = this.storage.getItem(ACCOUNT_LIST).map(item => decodeAccount(item, oldPassword))
        list = list.map(item => encodeAccount(item, newPassword))
        this.storage.setItem(ACCOUNT_LIST, list)
